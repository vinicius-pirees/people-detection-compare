...
def detect_single_frame(hog, frame, row, column, tiles_dict, **kwargs):
    ...
    detections, probs = non_max_suppression(detections, probs=probs, overlapThresh=0.65) #Non-Max Supression
...
if detect_on_tiles == 'y':
    write_cpu_usage_file(main_dir, 'VIRAT_videos', 'hog_confidence', str(tiles_x) + ',' + str(tiles_y))
else:
