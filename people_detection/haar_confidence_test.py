...
with open(os.path.join(videos_path, 'videos_to_process_test.txt')) as f:
...
def detect_single_frame(full_body_cascade, frame, row, column, tiles_dict, **kwargs):
    ...
    detections = full_body_cascade.detectMultiScale3(gray, scaleFactor, minNeighbors, minSize=(30, 30), flags=cv2.CASCADE_SCALE_IMAGE, outputRejectLevels=True)
    ...
    try:
        bodies, probs = non_max_suppression(bodies, probs=probs, overlapThresh=0.65)
    except:
        print(bodies)
        print(probs)
        raise ValueError
...
if detect_on_tiles == 'y':
    write_cpu_usage_file(main_dir, 'VIRAT_videos', 'haar_confidence', str(tiles_x) + ',' + str(tiles_y))
else:
    write_cpu_usage_file(main_dir, 'VIRAT_videos', 'haar_confidence', None)
